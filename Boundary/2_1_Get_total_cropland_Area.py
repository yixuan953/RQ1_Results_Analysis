...
input_path = '/lustre/nobackup/WUR/ESG/zhou111/Data/Raw/SPAM/geotiff_global_phys_area'
output_path = "/lustre/nobackup/WUR/ESG/zhou111/Data/Raw/SPAM/geotiff_global_phys_area"
...
for crop in crop_list:
    try:
        ...
        HA_file = os.path.join(input_path, f"SPAM2005V3r2_global_A_TA_{crop}_A.tif")
        ...
        ds = xr.Dataset(
            {
                "Physical_Area": (["lat", "lon"], HA_aggregated),
            },
            coords={
                "lon": lon_new,
                "lat": lat_new
        ...
        ds["Physical_Area"].attrs = {
            "units": "ha",
            "long_name": f"Physical area for {crop}",
            "_FillValue": np.nan,
        ...
        nc_file = os.path.join(output_path, f"{crop}_Physical_Area_05d.nc")
        encoding = {
            "Physical_Area": {"zlib": True, "complevel": 5},
        }
